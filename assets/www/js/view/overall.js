		...
		.on(
				...
				function() {
					...
					$page.on("swipeleft swiperight", function(e) {
						// We check if there is no open panel on the page
						// because otherwise
						// a swipe to close the left panel would also open the
						// right panel (and v.v.).
						// We do this by checking the data that the framework
						// stores on the page element (panel: open).
					...
					$(function() {
						...
						// data-icon='false'><a data-transition='slide' href=" +
						// link1 +"><img src=" + imageSrc1 +" />" +
						// "<h1>" + title1 + "</h1><p>" + subtext1 +
						...
						// $(".left-panel-data").append($("<li
						// data-icon='false'><a href=" + link2 +"><img src=" +
						// imageSrc2 +" />" +
						// "<h1>" + title2 + "</h1><p>" + subtext2 +
						// "</p></a></li>")).listview('refresh');
						...
						// $(".left-panel-data").append($("<li
						// data-icon='false'><a href=" + link3 +"><img src=" +
						// imageSrc3 +" />" +
						// "<h1>" + title3 + "</h1><p>" + subtext3 +
						// "</p></a></li>")).listview('refresh');
						...
						// $(".left-panel-data").append($("<li
						// data-icon='false'><a href=" + link5 +"><img src=" +
						// imageSrc5 +" />" +
						// "<h1>" + title5 + "</h1><p>" + subtext5 +
						// "</p></a></li>")).listview('refresh');

						$(".left-panel-data")
								.html(
										$("<li id='username-field' data-icon='false'>Stranger</li>"
												+ "<li data-icon='false'><a data-transition='slide' href="
												+ link0
												+ ">"
												+ "<img id='profile-link' style='margin-top:10px;margin-left:10px;width:60px;' src='css/global/images/"
												+ imageSrc0
												+ " />"
												+ "<h1>"
												+ title0
...
function initializeHome() {
	...
	// Modify slider
	$('.slider-font').css("fontSize",
			($('html').css('font-size').replace(/[^-\d\.]/g, '') * 0.6));
	...
	var leftoverHeight = window.innerHeight
			- $(".primary-header").innerHeight()
			- $(".secondary-header").innerHeight()
			- $(".pet-info").innerHeight();
...
function initializeMyRuns() {
	...
	// Resize run listing
	$('.sec-header-wording').attr("style", "text-align:center;");
	var leftoverHeight = window.innerHeight - $('.analytics-a').innerHeight()
			- $(".primary-header").innerHeight()
			- $(".secondary-header").innerHeight() - 35;
	$(".run-listing").height(leftoverHeight);
	// Resize run listing end
}
// MyRuns.html JS end
...
function initializeMap() {
	// Resize run listing
	$('.sec-header-wording').attr("style", "text-align:center;");
	var leftoverHeight = window.innerHeight - $('.map-position').innerHeight()
			- $(".primary-header").innerHeight() - 39
			- $(".secondary-header").innerHeight() - 35;
	...
	FB.login(function(response) {
		if (response.authResponse) {
			...
			FB.api('/me', function(response) {
				// var user1= new userProfile(response.first_name,
				// response.last_name,response.email,0,response.link +
				// "/picture");
				// user1.persist();
				localStorage.setItem("firstname", response.name);
				localStorage.setItem("photo", 'http://graph.facebook.com/'
						+ response.id + '/picture');
...
function addObjectToUnorderedList(url, img, header, content) {
	return '<li><a href="' + url + '"><img src="css/global/images/' + img
			+ '" /><h1>' + header + '</h1><p>' + content + '</p></a></li>';
}
